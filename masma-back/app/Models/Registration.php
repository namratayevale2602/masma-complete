...
class Registration extends Model
{
    ...
    /**
     * Find existing member with fallback options
     */
    public static function findExistingMemberWithFallback($email, $mobile, $name)
    {
        // Try exact match by email or mobile
        $member = self::where(function($query) use ($email, $mobile) {
                $query->where('office_email', $email)
                    ->orWhere('mobile', $mobile);
            })
            ->where('payment_verified', true)
        ...
        if (!$member && $name) {
            $member = self::where('applicant_name', $name)
                ->where('payment_verified', true)
                ->whereNotNull('member_id')
                ->orderBy('payment_verified_at', 'desc')
                ->first();
     ...
     */
    public static function isExistingMember($email, $mobile)
    {
        return self::where(function($query) use ($email, $mobile) {
                $query->where('office_email', $email)
                    ->orWhere('mobile', $mobile);
    ...
    /**
     * Get the original member record (the first verified registration)
     */
    public function getOriginalMember()
    {
        if ($this->parent_member_id) {
            return self::where('member_id', $this->parent_member_id)
                ->where('payment_verified', true)
                ->first();
        }
        return $this;
    ...
    /**
     * Check if membership is still valid (within 1 year)
     */
    public function isMembershipValid()
    {
        $expiryDate = $this->created_at->addYear();
